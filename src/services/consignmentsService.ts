...
/**
 * 100% Direct Supabase Postgres Fetch & Persistence for Consignments
 */
export async function fetchConsignments(): Promise<Consignment[]> {
  ...
  try {
    // 1. Try dedicated 'consignments' table first
    ...
    // 2. Fallback to 'orders' table where order_code starts with 'REM-'
    ...
    if (!oError && oData && oData.length > 0) {
      return oData.map((row) => {
        ...
        return {
          ...
          totalValue: Number(row.total_value) || 0,
          status: (row.status === 'Concluído' ? 'Concluída' : 'Em andamento') as any,
...
export async function createConsignment(consignment: Consignment): Promise<boolean> {
  ...
  try {
    // Try primary insert into 'consignments' table
    const payload = {
      code: consignment.id,
    ...
    const { error: cErr } = await supabase.from('consignments').insert([payload]);

    if (cErr) {
      console.warn('Tabela consignments inexistente ou aviso, salvando em orders como fallback:', cErr.message);

      // Fallback insert into 'orders' table with REM- prefix
      const orderPayload = {
        ...
        status: (consignment.status as string) === 'Finalizada' || (consignment.status as string) === 'Concluída' ? 'Concluído' : 'Em produção',
    ...
    console.error('Erro ao salvar consignação no Supabase:', err);
    return false;
  }
}

export async function syncMissingConsignmentsToSupabase(consignments: Consignment[]): Promise<number> {
  if (!isSupabaseConfigured() || consignments.length === 0) return 0;
  ...
  try {
    ...
    console.error('Erro ao sincronizar lote de consignações:', err);
  }
