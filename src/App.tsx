...
export function App() {
  ...
  const handleCompleteVisit = (visitData: any) => {
    ...
    // 1. Update clientInventories for this client with counts and new items
    ...
    // 2. Deduct restocked units from Workshop Stock (products.currentStock)
    ...
    // 3. Update Client Metrics (productsOnSiteCount and lastVisitDate)
    setClients((prev) =>
      prev.map((c) => {
        if (c.id === clientId) {
          return {
    ...
    // 4. Update Consignment Records (itemsCount and totalValue)
    setConsignments((prev) =>
      prev.map((c) => {
        const matchesClient =
          c.clientId === clientId ||
    ...
    // 5. If items were removed, create ExchangeNote
    ...
    // 6. Add financial transaction in Caixa
    if (received > 0) {
    ...
    // 7. Update or complete Visit in visits list
    setVisits((prev) =>
      prev.map((v) => {
