    ...
    <div className="space-y-6 animate-in fade-in duration-200">
        ...
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
          ...
          <div className="flex items-start gap-4">
            <div
              ...
              className={`w-14 h-14 rounded-2xl bg-indigo-600 text-white font-extrabold text-xl flex items-center justify-center shrink-0 shadow-sm overflow-hidden border border-indigo-200 ${currentClientData.avatarUrl ? 'cursor-zoom-in hover:scale-105 transition-transform' : ''
                }`}
          ...
          <div className="grid grid-cols-2 sm:flex sm:flex-wrap items-center gap-2 sm:gap-2.5 shrink-0 w-full lg:w-auto">
            ...
            {onDeleteClient && (
              <button
                ...
                className="col-span-2 sm:col-span-1 px-3.5 py-2.5 bg-rose-50 dark:bg-rose-950/40 hover:bg-rose-100 dark:hover:bg-rose-900/60 text-rose-700 dark:text-rose-300 border border-rose-200 dark:border-rose-800/80 rounded-xl text-xs font-bold shadow-xs flex items-center justify-center gap-1.5 transition-colors cursor-pointer"
                title="Excluir cadastro deste cliente"
              >
          ...
          {[
            ...
            return (
              <button
                ...
                className={`flex items-center justify-center gap-1.5 px-3.5 py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer ${isActive
                  ...
                  <div key={o.id} className="p-4 bg-slate-50 border border-slate-200 rounded-2xl space-y-2.5">
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <div className="flex items-center gap-1">
                        ...
                        <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold border transition-colors ${o.status === 'Entregue'
              ...
              <tbody className="divide-y divide-slate-100">
                {clientInventory.map((item) => {
                  ...
                  return (
                    <tr
                      key={item.productId}
                      className={`hover:bg-slate-50/80 transition-colors ${isLowTurnover ? 'bg-amber-50/30' : ''
                        }`}
                    >
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-200 pb-1">
                  ...
                  {editCepStatusMessage && (
                    <span className={`text-[11px] font-bold flex items-center gap-1 ${isSearchingEditCep ? 'text-indigo-600 animate-pulse' : editCepStatusMessage.includes('✅') ? 'text-emerald-600' : 'text-amber-600'
                      }`}>
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center p-4 z-50 animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#12151c] rounded-2xl max-w-md w-full p-6 space-y-5 border border-slate-200 dark:border-slate-800 shadow-2xl">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">Excluir Cliente</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400">Esta ação removerá o cadastro do cliente.</p>
              </div>
            </div>

            <div className="p-3.5 bg-rose-50/80 dark:bg-rose-950/20 border border-rose-200 dark:border-rose-900/40 rounded-xl space-y-2">
              <p className="text-xs text-rose-900 dark:text-rose-300 font-semibold">
                Tem certeza que deseja excluir o cliente <b>{currentClientData.name}</b>?
              </p>
              <p className="text-[11px] text-slate-600 dark:text-slate-400 leading-relaxed bg-white dark:bg-[#12151c] p-2.5 rounded-xl border border-slate-200 dark:border-slate-800">
