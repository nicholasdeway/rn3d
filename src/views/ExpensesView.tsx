    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white dark:bg-[#12151c] p-6 rounded-2xl border border-slate-200/80 dark:border-[#202531] shadow-xs">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <button
            onClick={() => setIsRecurringBillsModalOpen(true)}
            className="px-3.5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-xs transition-all cursor-pointer flex items-center gap-1.5 relative group"
            title="Gerenciar avisos e lembretes de contas fixas (DAS, UpSeller ERP, ChatGPT, etc.)"
          >
            <Bell className="w-4 h-4 text-indigo-200 group-hover:rotate-12 transition-transform" />
            <span>Contas Fixas & Lembretes</span>
            {billAlerts.filter((a) => !a.isPaidThisMonth && (a.isOverdue || a.daysRemaining <= 7)).length > 0 && (
              <span className="bg-rose-500 text-white text-[10px] font-extrabold px-1.5 py-0.2 rounded-full ml-1">
                {billAlerts.filter((a) => !a.isPaidThisMonth && (a.isOverdue || a.daysRemaining <= 7)).length}
              </span>
            )}
          </button>
      ...
      {/* MODAL: GESTÃO DE CONTAS FIXAS & LEMBRETES RECORRENTES */}
      {isRecurringBillsModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#12151c] border border-slate-200/80 dark:border-[#202531] rounded-3xl max-w-3xl w-full p-6 shadow-2xl space-y-5 flex flex-col max-h-[90vh]">
            <div className="flex items-start justify-between border-b border-slate-100 dark:border-slate-800/80 pb-4 shrink-0">
              <div className="flex items-center gap-3">
                <div className="p-3 bg-indigo-100 dark:bg-indigo-950/80 text-indigo-600 dark:text-indigo-400 rounded-2xl">
                  <Bell className="w-6 h-6" />
                </div>
                <div>
                  <h3 className="font-bold text-slate-900 dark:text-slate-100 text-lg">
                    Gestão de Contas Fixas & Lembretes
                  </h3>
                  <p className="text-xs text-slate-500 dark:text-slate-400">
                    Cadastre suas obrigações mensais (DAS, UpSeller, ChatGPT, etc.) e receba avisos em toda a plataforma.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-xs cursor-pointer flex items-center gap-1.5 transition-all"
                >
                  <Plus className="w-4 h-4" /> Nova Conta Fixa
                </button>

                <button
                  onClick={() => setIsRecurringBillsModalOpen(false)}
            ...
            {/* Quick Presets Bar */}
            <div className="p-3 bg-slate-50 dark:bg-slate-900/60 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shrink-0">
              <p className="text-[10px] font-extrabold uppercase tracking-wider text-slate-400 dark:text-slate-500 mb-2">
                Modelos Rápidos (1-Clique para Criar)
              </p>
              <div className="flex flex-wrap items-center gap-2">
                <button
                  onClick={() => {
                    ...
                    setBillFormData({
                      ...
                      amount: '250.00',
                      dueDay: '20',
                      beneficiary: 'Receita Federal',
                      notes: 'Guia DAS de imposto mensal da empresa',
                      recurrence: 'Mensal',
                    });
                    setIsNewBillModalOpen(true);
                  }}
                  className="px-2.5 py-1.5 bg-rose-50 dark:bg-rose-950/60 hover:bg-rose-100 text-rose-700 dark:text-rose-300 rounded-xl text-xs font-bold border border-rose-200 dark:border-rose-900 cursor-pointer flex items-center gap-1"
                >
                  💡 DAS Imposto
                </button>
                ...
                <button
                  ...
                  className="px-2.5 py-1.5 bg-indigo-50 dark:bg-indigo-950/60 hover:bg-indigo-100 text-indigo-700 dark:text-indigo-300 rounded-xl text-xs font-bold border border-indigo-200 dark:border-indigo-900 cursor-pointer flex items-center gap-1"
                >
                  💡 UpSeller ERP
                </button>
                ...
                <button
                  onClick={() => {
                    ...
                    setBillFormData({
                      ...
                      dueDay: '15',
                      beneficiary: 'OpenAI',
                      notes: 'Assinatura de inteligência artificial',
                      recurrence: 'Mensal',
                    });
                    setIsNewBillModalOpen(true);
                  }}
                  className="px-2.5 py-1.5 bg-emerald-50 dark:bg-emerald-950/60 hover:bg-emerald-100 text-emerald-700 dark:text-emerald-300 rounded-xl text-xs font-bold border border-emerald-200 dark:border-emerald-900 cursor-pointer flex items-center gap-1"
                >
                  💡 ChatGPT Plus
                </button>
              </div>
            ...
            <div className="overflow-y-auto space-y-3 flex-1 pr-1">
              {recurringBills.length === 0 ? (
                <div className="p-8 text-center text-slate-400 space-y-2">
                  ...
                  <p className="font-bold text-xs text-slate-700 dark:text-slate-300">Nenhuma conta fixa cadastrada ainda</p>
                  <p className="text-[11px] text-slate-400">Clique nos modelos rápidos acima ou no botão "+ Nova Conta Fixa".</p>
                </div>
              ) : (
                    ...
                    <div
                      ...
                      className={`p-4 rounded-2xl border transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-3 ${
                        isPaid
                          ...
                          : 'bg-white dark:bg-[#181c26] border-slate-200/80 dark:border-slate-700/80 shadow-xs'
                      }`}
                    >
                      <div className="space-y-1">
                        ...
                        <h4 className="font-extrabold text-sm text-slate-900 dark:text-slate-100">{bill.title}</h4>

                        {bill.beneficiary && (
                          <p className="text-xs text-slate-500 dark:text-slate-400">
                            Beneficiário: <span className="font-semibold text-slate-700 dark:text-slate-300">{bill.beneficiary}</span>
                          </p>
                        )}
                      </div>
                      ...
                      <div className="flex items-center justify-between sm:justify-end gap-3 shrink-0">
                        ...
                        <div className="flex items-center gap-1.5">
                          {!isPaid && onMarkBillPaid && (
                            <button
                              onClick={() => {
                                onMarkBillPaid(bill);
                              }}
                              className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-xl shadow-2xs transition-all cursor-pointer flex items-center gap-1"
                              title="Marcar como paga no mês e lançar no Financeiro"
                            >
                          ...
                          <button
                            onClick={() => {
                              if (onUpdateBill) {
                                onUpdateBill(bill.id, { status: bill.status === 'Ativo' ? 'Pausado' : 'Ativo' });
                              }
                            }}
                            ...
                            title={bill.status === 'Ativo' ? 'Pausar lembrete' : 'Ativar lembrete'}
                          >
                          ...
                          <button
                            onClick={() => {
                              if (onDeleteBill && confirm(`Deseja excluir a conta fixa "${bill.title}"?`)) {
                                onDeleteBill(bill.id);
                              }
                            }}
                            className="p-1.5 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-xl transition-colors cursor-pointer"
      ...
      {/* MODAL: CRIAR OU EDITAR CONTA FIXA RECORRENTE */}
      {isNewBillModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-md animate-in fade-in duration-200">
          <div className="bg-white dark:bg-[#12151c] border border-slate-200/80 dark:border-[#202531] rounded-3xl max-w-lg w-full p-6 shadow-2xl space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
              <div className="flex items-center gap-2">
                <Bell className="w-5 h-5 text-indigo-600 dark:text-indigo-400" />
                <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base">
                  {editingBill ? 'Editar Conta Fixa' : 'Nova Conta Fixa Recorrente'}
                </h3>
              </div>
              <button
            ...
            <form
              onSubmit={async (e) => {
                e.preventDefault();
                const amountVal = parseFloat(billFormData.amount.replace(',', '.'));
                const dueDayVal = parseInt(billFormData.dueDay, 10);
                if (isNaN(amountVal) || amountVal <= 0 || !billFormData.title.trim() || isNaN(dueDayVal)) return;

                if (editingBill && onUpdateBill) {
                  await onUpdateBill(editingBill.id, {
                    title: billFormData.title.trim(),
                    category: billFormData.category,
                    amount: amountVal,
                    dueDay: dueDayVal,
                    beneficiary: billFormData.beneficiary.trim(),
                    notes: billFormData.notes.trim(),
                    recurrence: billFormData.recurrence,
                  });
            ...
            >
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2.5 bg-slate-50 dark:bg-[#181c26] border border-slate-200 dark:border-slate-700/80 rounded-xl text-slate-900 dark:text-slate-100 font-semibold focus:outline-none focus:border-indigo-500"
                />
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">
                    Categoria do Financeiro *
                  </label>
                  <select
                    value={billFormData.category}
                    onChange={(e) => setBillFormData({ ...billFormData, category: e.target.value as ExpenseCategory })}
                    className="w-full px-3 py-2.5 bg-slate-50 dark:bg-[#181c26] border border-slate-200 dark:border-slate-700/80 rounded-xl text-slate-900 dark:text-slate-100 font-semibold focus:outline-none cursor-pointer"
                  >
                    {CATEGORIES.map((cat) => (
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBillFormData({ ...billFormData, amount: e.target.value.replace(/[^0-9.,]/g, '') })}
                    className="w-full px-3 py-2.5 bg-slate-50 dark:bg-[#181c26] border border-slate-200 dark:border-slate-700/80 rounded-xl text-slate-900 dark:text-slate-100 font-semibold focus:outline-none focus:border-indigo-500"
                  />
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setBillFormData({ ...billFormData, dueDay: e.target.value })}
                    className="w-full px-3 py-2.5 bg-slate-50 dark:bg-[#181c26] border border-slate-200 dark:border-slate-700/80 rounded-xl text-slate-900 dark:text-slate-100 font-semibold focus:outline-none focus:border-indigo-500"
                  />
                </div>
                ...
                <div>
                  <label className="block text-slate-700 dark:text-slate-300 font-bold mb-1">
                    Favorecido / Fornecedor / ERP
                  </label>
                  <input
                    type="text"
                    placeholder="Ex: Receita Federal, UpSeller, OpenAI..."
                    value={billFormData.beneficiary}
                    onChange={(e) => setBillFormData({ ...billFormData, beneficiary: e.target.value })}
                    className="w-full px-3 py-2.5 bg-slate-50 dark:bg-[#181c26] border border-slate-200 dark:border-slate-700/80 rounded-xl text-slate-900 dark:text-slate-100 font-semibold focus:outline-none focus:border-indigo-500"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  rows={2}
                  placeholder="Instruções de pagamento, código do boleto, login..."
                  value={billFormData.notes}
                  onChange={(e) => setBillFormData({ ...billFormData, notes: e.target.value })}
                  className="w-full px-3 py-2 bg-slate-50 dark:bg-[#181c26] border border-slate-200 dark:border-slate-700/80 rounded-xl text-slate-900 dark:text-slate-100 font-semibold focus:outline-none"
                />
              </div>
